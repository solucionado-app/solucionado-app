...
export default function Nav() {
    ...
    return (
        <>
          <div className="w-full bg-sol_darkBlue py-6 sm:py-4 px-4 sm:px-12  text-white" >
                <div className="flex justify-end md:justify-between items-center">
                <a className="hidden md:block" href="http://wa.me/5492994014514">+01 1234567890</a>
                <a className="hidden md:block" href="#">info@solucionado.com.ar</a>
            ...
            <nav className="w-full bg-solBlue py-8 px-4 sm:px-12 d_none text-white relative flex justify-between items-center ">

                <Link className="text-3xl font-bold leading-none flex items-center gap-2" href="/">
                    <Image src="/solucionado-transparente.png" height={35} width={140} className="object-contain" alt="logo solucionado" />

                </Link>

                <ul className={`hidden  absolute top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2 lg:flex lg:mx-auto  lg:items-center lg:w-auto lg:space-x-6`}>
                    <li><Link className={`text-sm sm:text-lg ${router.asPath === "/" ? "text-solYellow font-bold" : "text-gray-200"}  hover:text-sol_lightBLue`} href="/">Home</Link></li>

                    <li><a className={`text-sm sm:text-lg ${router.asPath === "/sobre-nosotros" ? "text-solYellow font-bold" : "text-gray-200"} hover:text-sol_lightBLue`} href="/sobre-nosotros">Sobre nosotros</a></li>


                    <li><Link className={`text-sm sm:text-lg ${router.asPath === "/contacto" ? "text-solYellow font-bold" : "text-gray-200"}  hover:text-sol_lightBLue`} href="/contacto">Contacto</Link></li>
                </ul>

                <div className="flex gap-4  items-center">
                    {!user.isSignedIn && <>
                        <Link className="hidden lg:inline-block py-2 px-4 bg-gray-50 hover:bg-gray-100 text-sm text-gray-900 font-bold  rounded-xl transition duration-200" href="/login">Iniciar Sesión</Link>
                        <Link className="hidden lg:inline-block py-2 px-4 bg-blue-500 hover:bg-blue-600 text-sm text-white font-bold rounded-xl transition duration-200" href="/registro">Registrarse</Link>
                    </>}
                    ...
                    <div id="burger" onClick={handleBurgerClick} className="lg:hidden">
                        <button className="navbar-burger flex items-center text-white hover:text-solYellow transition-colors p-3">
            ...
            <dialog ref={popover} className="top-0 transition-2 w-0 z-50  h-screen max-w-sm ml-0 mt-0 mb-0 p-0 " >
                ...
                <nav className="fixed top-0 left-0 bottom-0 flex opacity-0 flex-col w-9/12 max-w-sm py-4 px-4 transition-all duration-500 bg-gray-200 border-gray-400 border-r overflow-y-auto">
                    <div className="flex items-center mb-9 ">
                        <Link className="mr-auto text-3xl font-bold leading-none" href="/">
                            <svg className="h-10 rounded-full p-1" viewBox="0 0 10240 10240">
                        ...
                        <button onClick={closeDialog} className="navbar-close">
                            <svg className="h-6 w-6 text-black cursor-pointer hover:text-sol_darkBlue" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div>
                        <ul>
                            <li className="mb-1">
                                <Link className="block p-4 text-sm font-semibold text-zinc-900 hover:bg-slate-300 rounded" href="/">Home</Link>
                            </li>
                            <li className="mb-1">
                                <Link className="block p-4 text-sm font-semibold text-zinc-900 hover:bg-slate-300 rounded" href="/">Sobre Nosotros</Link>
                            </li>
                            <li className="mb-1">
                                <Link className="block p-4 text-sm font-semibold text-zinc-900 hover:bg-slate-300 rounded" href="/contacto">Contacto</Link>
                            </li>
                        </ul>
                    </div>
                    <div className="mt-auto">
                        {!user.isSignedIn && <div className="pt-6">
                            <Link className="block px-4 py-3 mb-3 leading-loose text-xs text-center font-semibold  bg-gray-50 hover:bg-gray-100 rounded-xl shadow" href="/login">Iniciar Sesión</Link>
                        ...
                        <p className="my-4 text-xs text-center text-zinc-900">
